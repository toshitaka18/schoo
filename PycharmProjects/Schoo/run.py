...
class User(db.Model): #どんなデータをどの型で格納するか記載
    __tablename__ = 'user'
    ...
    def login(email, password):
        ...
        ログイン実行
        '''
        u = User.query.filter_by(email=email).first()# queryを経由して、登録されているemailと入力されているemailが一致していれば、一つだけ返す
        if u and verify_password(u.password, password):# uが存在していて、かつ暗号化されたpasswordと暗号化されていないpasswordを照らし合わせてチェック
            return u
        return None
...
def index():
    ...
    if 'auth.user' in session:
        ...
        #　ログイン中のuser が登録されているidと等しい時、日付順に降順に並べて全て表示する
...
@app.route("/signup",methods=['GET','POST'])#signupのページを出すときはGET,データを送信するときはPOST
def signup():
    form = SignupForm()
    if form.validate_on_submit():
        u = User.query.filter_by(email=form.email.data).first()
        if u: #既に登録されているメールアドレスの時
            flash('そのメールアドレスは既に利用されています。')
            return redirect(url_for('.signup'))
        #　それ以外
        user = User()
        form.populate_obj(user)# データベースに保存するデータと画面から入力されたデータをまっぴんぐ
